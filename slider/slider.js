...
function sliderHandler(start, end, step, date) {
    ...
    document.querySelector(".prev").innerHTML = "-" + step + "ч.";
    document.querySelector(".end").innerHTML = end;
    document.querySelector(".next").innerHTML = "+" + step + "ч.";
    ...
    function createBlock(date, width) {
        ...
        dayBlock.innerHTML = document.querySelector(".slider-wrapper").innerWidth <= 525 ? `${addZero(date.getDate())}.${addZero(date.getMonth() + 1)}` : `${addZero(date.getDate())}.${addZero(date.getMonth() + 1)}.${date.getFullYear()}`;
